...
class LicenseBoxAPI{
    ...
    public function __construct(){
        $this->product_id = 'BF4F5C40';
        $this->api_url = 'https://license.schemamarkapp.com/';
        $this->api_key = 'E06689B6EDFD45879666';
    ...
    public function verify_license($time_based_check = false, $license = false, $client = false){
        ...
        if($time_based_check && $this->verification_period > 0){
            ...
            $today = date('d-m-Y');
            if(empty($_SESSION["e0066559d2dab8c"])){
                $_SESSION["e0066559d2dab8c"] = '00-00-0000';
            }
            if($type == 1){
                $type_text = '1 day';
            }elseif($type == 3){
            ...
            if(strtotime($today) >= strtotime($_SESSION["e0066559d2dab8c"])){
                $get_data = $this->call_api(
                    'POST',
